...
export const nameTranslationPrompts = {
  system: `你是一个专业的中英文名字翻译专家，擅长将外文名字转换为合适的中文名字。`,
  user: (name) => `请将英文名字"${name}"翻译成中文名字，要求音译与意译相结合，既保持发音相似性，又具有美好的中文含义。请以JSON格式返回结果。`
};

export const nameAnalysisPrompts = {
  system: `你是一个专业的姓名学分析师，擅长分析中文名字的各个维度。`,
  zh: (params) => `请分析中文名字"${params.name}"的含义、五行属性、文化背景等。`,
  en: (params) => `Please analyze the Chinese name "${params.name}" including its meaning, five elements, and cultural background.`
};

export const zodiacAnalysisPrompts = {
  system: `你是一个专业的生肖文化专家，擅长分析生肖与起名的关系。`,
